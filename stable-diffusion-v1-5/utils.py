...
def prepare_debug_output(tensor, resolution):
    ...
    tensor = ((tensor * 0.5 + 0.5).clamp(0, 1) * 255).permute(0,
                                                              1, 3, 4, 2).detach().cpu().numpy().astype("uint8")
...
def draw_valued_array(data, output_dir, grid_size=4):
    ...
    # Change cmap to your desired color map
...
def create_image_grid(args, images_array, captions=None):
    ...
    total_height = (thumbnail_height + caption_height +
                    spacing) * (len(images_array) // images_per_row)
    ...
    for i, (img_data, caption) in enumerate(zip(images_array, captions)):
        ...
        y = (i // images_per_row) * \
            (thumbnail_height + caption_height + spacing)
        ...
        draw.text((x, y + thumbnail_height),
                  f"{i:05d}_{wrapped_caption}", font=font, fill=(0, 0, 0))

    return grid_img


class SDTextDataset(Dataset):
    def __init__(self, anno_path, tokenizer_one=None, is_sdxl=False, tokenizer_two=None):
            ...
            with open(anno_path, "r") as f:
                for line in f:
                    ...
                    if "caption" in json_obj:
                        self.all_prompts.append(json_obj["caption"])
        else:
