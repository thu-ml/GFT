...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.log_snr = np.log(self.alphas_cumprod /
                              (1.0 - self.alphas_cumprod))
        self.lambda_t = np.exp(self.log_snr)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, teacher_model=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.args.rm_guidance:
                beta = th.rand(x_t.size(0)).to(x_t.device) * \
                        (self.args.max_beta - self.args.min_beta) + \
                        self.args.min_beta
                model_output, drop_ids = model(x_t, t, beta=beta.to(
                    x_t.device), return_drop_ids=True, **model_kwargs)
                model_output = model_output[:, :4]

                if self.args.distill:
                    teacher_output_cond = teacher_model(
                        x_t, t, **model_kwargs)[:, :4]

                model_kwargs['y'] = th.full_like(
                    model_kwargs['y'], 1000).to(x_t.device)

                if self.args.distill:
                    teacher_output_uncond = teacher_model(
                        x_t, t, **model_kwargs)[:, :4]
                else:
                    model.model.eval()
                    with th.no_grad():
                        model_output_uncond = model(x_t, t, beta=th.full(
                            (x_t.size(0), ), 1.0).to(x_t.device), **model_kwargs)[:, :4]
                    model.model.train()

                beta = beta.reshape(x_t.size(0), 1, 1, 1)
                if self.args.distill:  # standard distillation
                    target = teacher_output_uncond + beta * \
                        (teacher_output_cond - teacher_output_uncond)
                else:
                    interpolated_output = beta * model_output + \
                        (1 - beta) * model_output_uncond.detach()

                if self.args.distill:
                    terms["loss"] = mean_flat(
                        (target - model_output) ** 2)
                    terms["loss_psample"] = mean_flat(
                        (noise - model_output) ** 2)
                else:
                    terms["loss"] = mean_flat(
                        (noise - interpolated_output) ** 2)
                    terms["loss_uncond"] = mean_flat(
                        (noise - model_output_uncond.detach()) ** 2)  # log uncond_loss, no grad
                    terms["loss_psample"] = mean_flat(
                        (noise - model_output.detach()) ** 2)
                return terms
