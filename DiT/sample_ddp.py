...
def main(args):
    ...
    using_cfg = args.cfg_scale > 1.0 and args.rm_guidance is False
        ...
        if using_cfg:
            ...
            if args.rm_guidance:
                if args.distill:
                    ...
                    beta = 1.0 / args.cfg_scale
                model_kwargs.update(
                    {"beta": torch.full(y.shape, beta, device=device)})
            sample_fn = model.forward

        if args.dpm_solver:
            samples = dpm_sample.sample(
                sample_fn, args.num_sampling_steps, z, device, "dpmsolver++", model_kwargs)
        else:
            # Sample images:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, required=True, default="/path/to/ImageNet/train")
    parser.add_argument("--sample-dir", type=str,
                        default="samples/DiT-XL/ddpm-250")
    parser.add_argument("--dpm-solver", action='store_true')
