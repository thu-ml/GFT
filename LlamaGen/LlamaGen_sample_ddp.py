...
def main(args):
    ...
    if args.from_fsdp:
        ...
        path_parts = args.gpt_ckpt.replace(".pth", "").replace(".pt", "").split('/')
        ckpt_string_name = f"{path_parts[-3]}-{path_parts[-1]}"
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.image_size}-size-{args.image_size_eval}-{args.vq_model}-" \
                  ...
                  f"cfg-{args.cfg_scale}-seed-{args.global_seed}-beta-{args.beta}"
    ...
    for _ in pbar:
        ...
        index_sample = generate(
            ...
            top_p=args.top_p, sample_logits=True, beta=args.beta,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global-seed", type=int, default=23333)
    parser.add_argument("--top-k", type=int, default=0,help="top-k value to sample with")
    parser.add_argument("--temperature", type=float, default=1.0, help="temperature value to sample with")
    parser.add_argument("--top-p", type=float, default=1.0, help="top-p value to sample with")
    parser.add_argument("--beta", type=float, default=None, help="top-p value to sample with")
